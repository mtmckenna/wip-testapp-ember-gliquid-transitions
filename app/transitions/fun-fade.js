...
export default function funFade(opts={}) {
  ...
  var convertNewElementToImage = D2I.toPng(rawNewElement).then((dataUrl) => {
    var img = new Image();
    img.src = dataUrl;
    document.body.appendChild(img);
    return img;
  ...
  RSVP.hash({
    ...
    console.log(hash);
    doTheThing(hash.fromImage, hash.toImage);
  });

  return Promise.all([
    convertOldElementToImage,
    convertNewElementToImage
  ]);
}

function doTheThing(fromImage, toImage) {
...
/*export default function funFade(opts={}) {
  ...
  var rawNewElement = this.newElement.get(0);
  this.newElement.css({visibility: ''});
  var toImagePromise = D2I.toPng(rawOldElement).then((dataUrl) => {
    var img = new Image();
    img.src = dataUrl;
    document.body.appendChild(img);
  }).catch(function (error) {
